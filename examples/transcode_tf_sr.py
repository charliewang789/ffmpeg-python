...
#.input(in_filename, hwaccel='qsv', **{'c:v': 'h264_qsv'})
#-vf 'vpp_qsv=framerate=60,scale_qsv=w=1920:h=1080:format=rgb32,hwdownload,format=rgb32'
#  .output('pipe:', vf='hwdownload,format=nv12', pix_fmt='rgb24')
...
def start_ffmpeg_process1_dxva2(in_filename):
    logger.info('Starting ffmpeg process1')
    args = (
        ffmpeg
        .input(in_filename, hwaccel='dxva2')
        #.input(in_filename, hwaccel='qsv', **{'c:v': 'h264_qsv'})
        .output('pipe:', format='rawvideo', pix_fmt='rgb24')
        .compile()
    )
...
def start_ffmpeg_process1_qsv(in_filename):
    logger.info('Starting ffmpeg process1')
    args = (
        ffmpeg
        .input(in_filename, hwaccel='qsv', **{'c:v': 'h264_qsv'})
...
#.input('pipe:', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(width, height),**{'c:v': 'h264_qsv'})
...
def start_ffmpeg_process2_qsv(out_filename, width, height):
    logger.info('Starting ffmpeg process2')
    args = (
        ffmpeg
        #.input('pipe:', init_hw_device='qsv=hw', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(width, height))#,**{'c:v': 'h264_qsv'})
        .input('pipe:', init_hw_device='qsv=hw', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(width, height))
        .output(out_filename, pix_fmt='nv12' ,**{'c:v': 'h264_qsv'}, **{'b:v':'5M'})
        .overwrite_output()
        .compile()
    )
    return subprocess.Popen(args, stdin=subprocess.PIPE)
